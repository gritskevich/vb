...
class BrowserManager {
    ...
    async handleNavigation(url) {
        try {
            ...
            // Validate page after navigation
            const isPageValid = await this.validatePage();
            if (!isPageValid) {
                console.log('Page validation failed, retrying navigation...');
            ...
            await this.handleNavigationError(url);
        }
    }
    ...
    async handleNavigationError(url) {
        console.log('Handling navigation error for:', url);
        try {
            await this.resetPage();
            ...
            console.error('Recovery navigation failed:', error);
            // If recovery fails, navigate to a known good page
